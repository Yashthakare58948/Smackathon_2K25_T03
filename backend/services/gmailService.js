...
class GmailService {
  ...
  async createOAuth2Client(userId) {
    try {
      ...
      if (!tokenData) {
        throw new Error("No Gmail token found for this user. Please authenticate with Gmail first.");
      }
      ...
      if (Date.now() >= tokenData.expiry_date) {
        ...
        if (!refreshedToken) {
          throw new Error("Gmail token has expired. Please re-authenticate with Gmail.");
        }
      ...
      await GmailToken.findByIdAndUpdate(tokenData._id, { lastUsed: new Date() });
  ...
  async listExpenseEmails(userId, searchQueries = null) {
    try {
      ...
      const defaultQueries = [
        ...
        `after:${Math.floor(Date.now() / 1000) - 30 * 24 * 60 * 60} (expense OR receipt OR payment)`, // Last 30 days
      ];
      ...
      for (const query of queries) {
        ...
        if (res.data.messages && res.data.messages.length > 0) {
          console.log(`Found ${res.data.messages.length} messages with query: "${query}"`);
