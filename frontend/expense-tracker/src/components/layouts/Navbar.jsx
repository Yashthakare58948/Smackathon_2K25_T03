...
const Navbar = ({ activeMenu }) => {
  ...
  const handleGmailImport = async () => {
    ...
    try {
      ...
      toast.success(
        `Import completed! ${data.totalExpensesImported} expenses imported, ${data.duplicatesSkipped} duplicates skipped.`,
        { id: "gmail-import" }
      );

      console.log("Gmail import result:", data);

      // Optionally refresh the page or update expenses list
      // You can add a callback prop to refresh expenses if needed
    } catch (error) {
